...
- DM排班台：店长按日期建立 14:00 / 19:00 班次并填写所需技能与人数；DM 只可认领未满且符合技能的班次，同一时段不能重复上岗；换班须由同技能、当日空档的 DM 承接，双方确认后两张排班同时更新，任一步失败都保持原状；页面展示班次缺口、我的班次和待处理换班，满员或冲突时说明原因，数据落库刷新不丢。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/schedule/board?date=YYYY-MM-DD&dmId=` | 排班看板（班次缺口、我的班次、待处理换班） |
| POST | `/api/schedule/shifts` | 店长建班（日期、时段、技能、人数） |
| POST | `/api/schedule/shifts/{id}/claim` | DM 认领班次 |
| POST | `/api/schedule/swaps` | 发起换班申请 |
| POST | `/api/schedule/swaps/{id}/accept` | 承接换班 |
| POST | `/api/schedule/swaps/{id}/confirm` | 发起人确认，双方排班同时更新 |
| POST | `/api/schedule/swaps/{id}/cancel` | 取消申请 / 承接人撤回 |
