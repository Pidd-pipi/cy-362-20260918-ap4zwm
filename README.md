...
- DM排班台（`/scheduling`）：店长按日期建立 14:00 / 19:00 班次并指定所需技能与人数；DM 仅可认领未满员且技能匹配的班次，同一时段不能重复上岗；支持空档代班与同技能对调换班，双方确认后两张排班在同一事务内同时更新，任一步失败全部回滚保持原状；页面展示班次缺口、我的班次与待处理换班，并对满员、技能不符、时段冲突给出明确原因；排班数据落库，刷新或重启后记录仍在。
